...
abstract contract AminalDescriptor is NFTDescriptor {
    ...
    function dataURI(uint256 tokenId) public view returns (string memory) {
        ...
        string memory image = string(abi.encodePacked('data:image/svg+xml;base64,', Base64.encode(bytes(_aminalImage(tokenId )))));
     ...
     */
    function _aminalImage(uint _tokenId) internal pure returns (string memory output) {
        ...
        output = string(abi.encodePacked(output, '</text><text x="240" y="320" class="base">aminal # ' ,toString(_tokenId) , '</text></svg>'));
    }
    ...
    function monthStringShort (uint month) public pure returns (string memory monthString) {
    ...
    function dayOfMonthString (uint dayOfMonth) public pure returns (string memory dayString) {
